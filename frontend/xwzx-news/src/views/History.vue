<template>
  <div class="history-container">
    ...
    <div class="history-list">
      <van-list
        v-model:loading="historyStore.listLoading"
        :finished="!historyStore.hasMore"
        :finished-text="historyStore.getHistory.length ? '没有更多了' : ''"
        @load="onLoad"
      >
        ...
        <van-empty
          v-if="!historyStore.listLoading && historyStore.getHistory.length === 0"
          description="暂无浏览历史"
        />
      </van-list>
    </div>
...
// 上拉加载更多（store 内部对并发请求自动去重，hasMore=false 时 van-list 自动终止）
const onLoad = async () => {
  try {
    const result = await historyStore.getHistoryListApi();

    // 未登录或请求失败：回退本地数据（本地数据不分页）
    if (!result || !result.success) {
      historyStore.loadHistory();
      historyStore.hasMore = false;
    }
  } catch (error) {
    console.error('浏览历史页面：加载失败', error);
    historyStore.loadHistory();
    historyStore.hasMore = false;
  }
};
...
// 组件挂载时加载历史记录
onMounted(() => {
  // 首次进入显式拉取第一页；van-list 的自动 @load 会被 store 的在途请求去重，不会重复请求
